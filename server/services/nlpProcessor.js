...
function extractEntities(text) {
  ...
  // ── Component names lookup (authoritative fallback) ──
  // Only runs when regex patterns found nothing with ≥ 0.88 confidence,
  // preventing short component names like "Studio" from beating a longer
  // regex match like "visual studio code".
  const hasRegexMatch = [...entities.os, ...entities.software, ...entities.device]
    .some((e) => (confidences[e] || 0) >= 0.88);

  if (!hasRegexMatch) {
    const componentMatch = findComponentInText(text);
    if (componentMatch) {
      const cName = componentMatch.name.toLowerCase();
      if (!entities.software.includes(cName)) {
        entities.software.push(cName);
        confidences[cName] = componentMatch.confidence;
        console.log(`[DEBUG][NLP] Component match: "${componentMatch.name}" (${componentMatch.confidence})`);
      }
    }
  }
...
const BREAKING_TYPE_MAP = [
  { pattern: /\b(critical\s+failure|critical\s+break)\b/,                           apiType: "Critical Failure",             queryType: "critical" },
  { pattern: /\b(breaking\s+update|breaking\s+change|breaking\s+release)\b/,        apiType: "Breaking Update",              queryType: "breaking" },
  { pattern: /\b(compatibility\s+issue|compatibility\s+problem|incompatib)\b/,       apiType: "Compatibility Issues",         queryType: "breaking" },
  { pattern: /\b(network\s+issue|network\s+failure|network\s+problem)\b/,            apiType: "Network Issues",               queryType: "breaking" },
  { pattern: /\b(logging.{0,10}monitoring|log\s+failure|monitoring\s+fail)\b/,       apiType: "Logging & Monitoring Failures", queryType: "breaking" },
  { pattern: /\b(config(?:uration)?\s+error|config(?:uration)?\s+issue)\b/,          apiType: "Configuration Errors",         queryType: "breaking" },
...
// Converts "yesterday", "today", "this month", "last month", "this week",
// "last week", "last N days" into { type, date/from/to, displayDate, label }.
